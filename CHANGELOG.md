...
- `IskakINO_Logger.h` — logging printf-style dengan flag debug per-instance (diekstrak dari ArduFast).
  - **Bug fix (ditemukan lewat CI sungguhan `arduino:avr:uno`):** avr-libc `<math.h>` (ke-include transitif lewat `Arduino.h`) mendefinisikan `log` sebagai MACRO ke `logf` (karena di AVR `double`==`float`). Ini bikin method `log()` di `IskakINO_Logger` (dan `IskakINO_ArduFast`, yang punya pola sama) diam-diam "dibajak" jadi `logf()` saat preprocessing, bentrok-ambigu dengan method `logf()` (variadic) yang sudah ada — `error: call of overloaded 'log(...)' is ambiguous`. Diperbaiki dengan `#undef log` (di-guard `#ifdef`, aman di platform yang tidak punya macro ini) di `core/IskakINO_Logger.h` — fix terpusat di sini otomatis berlaku ke semua modul yang compose Logger (Storage, LCD, SmartVoice), bukan cuma ArduFast.
